...
const saveBudgetSettings = async () => {
  ...
  try {
    const saved = await budgetStore.saveSettings({
    ...
    error.value = err.message || 'Could not save your budget settings. Try again.'
  } finally {
    ...
    >
      ...
      <div class="flex flex-col-reverse gap-2 border-t border-gray-100 bg-gray-50 px-3 py-3 sm:flex-row sm:justify-end sm:gap-2">
        <button
          type="button"
          :disabled="submitting"
          class="w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 transition hover:bg-gray-100 disabled:opacity-50 sm:w-auto"
          @click="closeModal"
        >
          Cancel
        </button>
        ...
        <button
          type="button"
          :disabled="submitting"
          class="w-full rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-emerald-700 hover:shadow-md active:scale-[0.98] disabled:opacity-60 sm:w-auto"
